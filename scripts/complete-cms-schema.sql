...
-- Admin policies (full access for authenticated users with admin profile)
CREATE POLICY "Admin full access" ON site_settings FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
CREATE POLICY "Admin full access" ON hero_section FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
CREATE POLICY "Admin full access" ON about_section FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
...
CREATE POLICY "Admin full access" ON tools FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
...
CREATE POLICY "Admin full access" ON certifications FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
CREATE POLICY "Admin full access" ON animated_stats FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
CREATE POLICY "Admin full access" ON contact_messages FOR ALL USING (
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
CREATE POLICY "Admin profiles access" ON admin_profiles FOR ALL USING (
    user_id = auth.uid() OR EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
...
CREATE POLICY "Admin upload access" ON storage.objects FOR INSERT WITH CHECK (
    bucket_id IN ('portfolio-images', 'portfolio-documents', 'portfolio-videos') AND
    EXISTS (SELECT 1 FROM admin_profiles WHERE user_id = auth.uid() AND is_active = true)
);
